...
const Blog = ({ posts: propPosts, initialCount = 9, perPage = 9 }) => {
    ...
    const posts = propPosts || config.posts || (document?.type === 'list' ? document.items : null) || fetchedPosts || [];

    React.useEffect(() => {
        if (posts.length === 0 && config.uri && db) {
             db.fetch(config.uri).then(data => {
        ...
        <section className="posts container my-5">
            {visiblePosts.map((post, index) => (
                <article key={index}>
                    <figure>
                        <a href={post.href}>
                            <img src={post.image || '/img/logo.png'} alt={post.title} />
                        </a>
                    </figure>
                    <header>
                        <div className="date">
                        ...
                        <div className="excerpt small text-muted">
